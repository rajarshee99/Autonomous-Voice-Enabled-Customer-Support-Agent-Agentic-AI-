...
[Overview](#-overview) · [Architecture](#-architecture) · [Quick Start](#-quick-start) · [API Reference](#-api-reference) · [Configuration](#-configuration) · [Project Structure](#-project-structure)
...
│
├── architecture/
│   ├── level 1 dfd.drawio          # Level 1 data flow diagram
│   └── level 2 dfd.drawio          # Level 2 data flow diagram
│
...
- [ ] **Graph-RAG** — Neo4j relationship mapping across crawled documents for multi-hop reasoning
- [ ] **Streaming TTS** — Real-time voice response via ElevenLabs or Bark
- [ ] **Local LLM fallback** — Ollama integration for offline/low-latency resolution
- [ ] **Celery/Redis workers** — Move crawler and chunker processes to a proper task queue
- [ ] **Session persistence** — Durable session storage (Redis) replacing module-level globals
